...
Do NOT critique the design or propose a better approach — that is not your job. You check only whether the plan's factual claims about the codebase are true. The one exception is the scope observation below, which is a report to the user, not a verdict on the plan.
